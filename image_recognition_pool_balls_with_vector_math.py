...
while True:
	...
	if len(cue_cnts) > 0 and len(eight_cnts) > 0:
    	...
    	eight_Contour = max(eight_cnts, key=cv2.contourArea)
		((x_eight, y_eight), radius_eight) = cv2.minEnclosingCircle(c)
		eight_M = cv2.moments(eight_Contour)
                # finds the center of a blob
		eight_center = (int(eight_M["m10"] / eight_M["m00"]), int(eight_M["m01"] / eight_M["m00"]))
                # uses formula to calculate centroid
    ...
    """
    hole = np.arange([600,600])
    eight = np.asarray(eight_center)
    holetoeight = hole - eight
    distance_8_0 = (holetoeight[0]**2+holetoeight[1]**2)**(0.5)
    desire_point = hole - (holetoeight * (distance_8_0 + radius_eight + radius_cue) / distance_8_0)
    desire_point.tolist()
    hole.tolist()

    cv2.line(frame, eight_center, hole, (0, 0, 255), thickness)
    cv2.line(frame, desire_point, cue_center, (0, 0, 255), thickness)
